...
1. Create a new game file under games. You can see some examples in the repo. A JSON game object has two keys:
    ...
    - `clues`. This is a list of clues. Each clue is a list of rule objects, each with two keys: `func` and `args`.
        - `func`. The available functions are these:
            ...
            - math functions:
              ...
              - These can all be prefixed by `!` to stipulate their negation.
        - `args`. A list of arguments. These are symbols or, for the boolean operators, rules. A symbol is the name of a thing, e.g. `Jason`, or a relationship, e.g. `Jason::Food` (the `Food` asssociated with `Jason`). Symbolic relationships can be multiple levels deep if needed.

2. Once you've defined your game file, run `main.py` and select your game. All possible solutions will be presented one at a time.

---

*Note that when evaluated, all the rules in a clue are joined by an implicit `and`. Clues are thus a redundant structure in terms of the logic, but exist to match the original puzzles' organization, where sometimes a single "clue" actually contains multiple rules.*

## Example

Let's look at [`timetoquit.pdf`](src/games/sources/timetoquit.pdf). A complete game file is provided in [`timetoquit.json`](src/games/sources/timetoquit.json); this tutorial will walk you through how it was made.

First, the kinds are as follows:
...
This clue means that each of these five things must be associated with a different person. No two can be associated with the same person. Thus, the function we want is `!link`, which means none of these things are paired with each other.
...
This is the same sort of logic as the second clue:
...
Note that having multiple rules in a clue is the same as using `and` to join subrules, but it requires less nesting.

Also, consider that saying `!link` with the arguments `Trenton` and `Apples` is the same as saying `!same` with the arguments `Trenton::Fruit` and `Apples` or with the arguments `Trenton` and `Apples::Person`. Our functions give us more than one way to express a relationship. You might observe that `same` is more specific (and it also runs faster), but `link` requires less writing. It would have been much harder to translate our first clue above without `link`, because we would have had to explicitly enumerate every pair of relationships.
...
* Remove the `progressbar2` dependency, providing a simple print progress statement if it's not present.
